...
class PoissonEngine:
    ...
    def calculate_probabilities(self, home_stats, away_stats):
        ...
        Args:
            home_stats: {'scored': float, 'conceded': float} (averages)
            away_stats: {'scored': float, 'conceded': float} (averages)
        
        Returns:
            dict with probabilities and expected goals (lambda)
        """
        # 1. Attack and Defense Strength
        # If team scores 1.3 (equal to average), strength is 1.0
